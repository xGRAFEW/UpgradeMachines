...
### อัปเดต v1.14.0 (2026-09-14) — ทำให้ความกว้าง/รูปแบบปุ่ม Dialog ตั้งค่าได้เองผ่าน config
ผู้ใช้ทดสอบ v1.13.1 (ปุ่มยกเลิกแยกลงล่างสุดผ่าน `exitAction`) ส่งภาพมา — ปุ่ม "ยืนยันอัพเกรด"/"ยกเลิก" ยาวเกือบเต็มความกว้างจอ (เพราะ hardcode `.width(300)` ไว้ตอนแก้ v1.13.0 เผื่อ label 2 บรรทัด) ขอให้ปรับให้พอดีกับตัวอักษร จากนั้นถามต่อว่าอยากปรับเองผ่าน config ได้ไหมโดยไม่ต้องรออัปเดต jar ทุกรอบ แล้วถามต่ออีกว่าปรับ**ตำแหน่ง**ได้ไหมด้วย
- อธิบายข้อจำกัดให้ผู้ใช้ฟังก่อนว่า Dialog ของ Minecraft เป็นระบบ grid/แถว ไม่ใช่พิกเซล x/y อิสระ เลยทำได้แค่ให้เลือกจาก preset ที่มีจริงเท่านั้น ไม่ใช่ปรับตำแหน่งอิสระ 100%
- เพิ่ม `UpgradeManager.DialogButtonLayout` enum (`SIDE_BY_SIDE`/`STACKED`/`SEPARATED`) + `getDialogButtonLayout()` อ่านจาก `gui.dialog.button-layout` (string, default `"separated"`) — รวม 3 แบบที่เคยลองทำมาทั้งเซสชันนี้ (v1.12.0 side-by-side ดั้งเดิม, v1.13.0 stacked ติดกัน, v1.13.1 separated แยกลงล่าง) เข้าไว้ใน config เดียว สลับได้เองไม่ต้องรอโค้ดใหม่
- เพิ่ม `getDialogButtonWidth()` อ่านจาก `gui.dialog.button-width` (int, default ลดจาก 300 → 200) — แก้ปัญหาปุ่มยาวเกิน + ให้ผู้ใช้ปรับเองได้ในอนาคตถ้า label ยาวขึ้น/สั้นลง
- แก้ `UpgradeConfirmDialog.java`: ใช้ `switch` เลือก `DialogType` ตาม `getDialogButtonLayout()` (SIDE_BY_SIDE→`confirmation()`, STACKED→`multiAction` 2 ปุ่มในกลุ่มหลัก, SEPARATED→`multiAction` ปุ่มเดียว+`exitAction`) แทนที่จะ hardcode แบบเดียวแบบเดิม
- อัปเดต config.yml ตัวอย่าง + comment อธิบายทั้ง 3 แบบ
- bump เวอร์ชันเป็น 1.14.0
- **ยังไม่ได้ทดสอบบนเซิร์ฟทดสอบท้องถิ่นอีกเช่นกัน (รอบที่ 3 ติดต่อกัน)**: แรมเครื่องยังต่ำมากต่อเนื่อง (เช็คแล้วเหลือ 0.79GB จาก 15.36GB ตอนนี้) ใช้แค่ build สำเร็จ (compile-verified) เป็นหลักฐานเหมือน 2 รอบก่อน
- Push + สร้าง Release `v1.14.0` แล้ว: https://github.com/xGRAFEW/UpgradeMachines/releases/tag/v1.14.0
- **ยังไม่ได้อัปเดตไฟล์ config ของผู้ใช้ที่ `C:\Users\ACER\Desktop\UpgradeMachines\config.yml` ให้มี `button-layout`/`button-width` ใหม่** — ควรเพิ่มให้ในรอบถัดไปที่คุยกัน (หรือถ้า session หน้าเจอ ให้เพิ่มสองค่านี้เข้าไปในไฟล์นั้นด้วย ก่อนบอกผู้ใช้ว่าพร้อมทดสอบ)
- **ยังไม่ได้ยืนยันจากผู้ใช้ว่าความกว้างปุ่ม 200 พอดีจริงไหม** รอผู้ใช้อัปเดต jar แล้วทดสอบ/ปรับค่าเองต่อ

### อัปเดต v1.13.1 (2026-09-14) — ปุ่มยกเลิกแยกลงไปด้านล่างสุด (exitAction)
ต่อจาก v1.13.0 (เรียงปุ่มแนวตั้ง 1 คอลัมน์ ปุ่มยืนยัน+ยกเลิกติดกัน) ผู้ใช้ขอเพิ่มเติม: **ปุ่มยืนยันให้อยู่ตำแหน่งเดิม ส่วนปุ่มยกเลิกให้แยกลงไปอยู่ด้านล่างสุด** (ไม่ใช่แค่เรียงต่อกัน) ถามว่าทำผ่าน config ได้ไหม — อธิบายว่าเรื่อง layout ทำผ่าน config ไม่ได้ ต้องแก้โค้ดเท่านั้น (ตอบซ้ำหลายรอบเพราะผู้ใช้ถามย้ำ) ผู้ใช้ยืนยัน "โอเคดำเนินการเลย"
- เจอว่า `DialogType.multiAction(actions, exitAction, columns)` มีพารามิเตอร์ **`exitAction` แยกต่างหาก**จาก `actions` (ปุ่มหลัก) — Minecraft เรนเดอร์ `exitAction` เป็นปุ่มแยกออกมาต่างหาก (มักจะอยู่ด้านล่างสุด แยกจากกลุ่มปุ่มหลัก) ตรงกับที่ต้องการเป๊ะ
- แก้ `UpgradeConfirmDialog.java`: ปุ่มยืนยันอยู่ใน `actions` list (แค่ปุ่มเดียว) ส่วนปุ่มยกเลิกย้ายไปที่ `.exitAction(cancelButton)` แทน
- bump เวอร์ชันเป็น 1.13.1
- **ไม่ได้ทดสอบบนเซิร์ฟทดสอบท้องถิ่นอีกเช่นกัน**: แรมเครื่องแย่ลงเรื่อย ๆ ทุกรอบที่เช็ค (v1.12.1: 2.55GB → v1.13.0: 1.28GB → v1.13.1: **เหลือแค่ 0.71GB** จาก 15.36GB) ข้ามการทดสอบท้องถิ่นไปเลยรอบนี้เช่นกัน ใช้แค่ build สำเร็จ (compile-verified) เป็นหลักฐาน — **สังเกตแนวโน้มแรมลดลงเรื่อย ๆ ต่อเนื่องหลาย session ในวันเดียวกัน (2026-09-14) ควรเตือนผู้ใช้เรื่องนี้ตรง ๆ ถ้า session หน้ายังเจอแรมต่ำอยู่ อาจมีโปรแกรมอื่นรั่วแรมสะสมบนเครื่องนี้ ไม่ใช่แค่ launch เซิร์ฟทดสอบของเราเอง**
- Push + สร้าง Release `v1.13.1` แล้ว: https://github.com/xGRAFEW/UpgradeMachines/releases/tag/v1.13.1
- **ยังไม่ได้ยืนยันจากผู้ใช้ว่าปุ่มยกเลิกแยกลงไปด้านล่างสุดจริงตามที่ต้องการไหม** (ยังไม่มีการันตีว่า `exitAction` เรนเดอร์ตรงตำแหน่งที่คาดไว้เป๊ะ ๆ - แค่คาดจากชื่อ/การใช้งานทั่วไปของ dialog ประเภทอื่นใน Minecraft เช่น "server links" ที่มีปุ่ม exit แยกจาก list หลัก) รอผู้ใช้อัปเดต jar แล้วส่งภาพมายืนยัน
